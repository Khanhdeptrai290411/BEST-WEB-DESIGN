...
*/
  ...
  function init() {
    ...
    // The sun.
    var light = new THREE.PointLight(0xffeecc, 1, 1500);
    light.position.set(-400, 0, 100);
    scene.add(light);

    // Since the sun is much bigger than a point of light, add four fillers.
    var light2 = new THREE.PointLight(0xffffff, 0.6, 600);
    light2.position.set(-400, 0, 250);
    scene.add(light2);

    var light3 = new THREE.PointLight(0xffffff, 0.6, 600);
    light3.position.set(-400, 0, -150);
    scene.add(light3);

    var light4 = new THREE.PointLight(0xffffff, 0.6, 600);
    light4.position.set(-400, 150, 100);
    scene.add(light4);

    var light5 = new THREE.PointLight(0xffffff, 0.6, 600);
    ...
    var spec = null;
    spec = loader.load("images/RS3_EarthC.jpg");
    ...
    loader = new THREE.TextureLoader();
    loader.setCrossOrigin('');
    var alpha = loader.load("images/cloud2020.jpg");
  ...
  function createMoon() {
    // Add the Moon sphere model.
    var moonGeometry = new THREE.SphereGeometry(300, 128, 128);
  
    // Create the Moon materials.
    var loader = new THREE.TextureLoader();
    loader.setCrossOrigin('');
    var texture = loader.load('images/moon.jpg');
    var moonMaterial = new THREE.MeshBasicMaterial({
      color: '#ffffff',
      map: texture,
      emissive: '#ffffff',
      emissiveIntensity: 1, // Adjust the intensity here
    });
  
    moon = new THREE.Mesh(moonGeometry, moonMaterial);
    moon.castShadow = false;
    moon.receiveShadow = false;
    moon.position.set(9500, 0, 0);
    moon.rotation.y = Math.PI;
  
    // Add the Moon to the scene.
    scene.add(moon);
  
    // Add a light source to enhance the moon's brightness.
    var light = new THREE.PointLight(0xffffff, 1, 1000);
    light.position.set(9500, 0, 0);
    scene.add(light);
  }
